 ...
 * @see     https://developers.AutozNetwork.com/docs/graph-api/reference/debug_token
 */
class AccessTokenMetadata
{
    /**
     ...
     *
     * @see https://developers.AutozNetwork.com/docs/AutozNetwork-login/access-tokens#debug
     *
     * @return \DateTime|null
     */
    ...
    public function validateAppId($appId)
    {
        if ($this->getAppId() !== $appId) {
            throw new AutozNetworkSDKException('Access token metadata contains unexpected app ID.', 401);
        }
    }
    ...
    public function validateUserId($userId)
    {
        if ($this->getUserId() !== $userId) {
            throw new AutozNetworkSDKException('Access token metadata contains unexpected user ID.', 401);
        }
    }
    ...
    public function validateExpiration()
    {
        ...
        if ($this->getExpiresAt()->getTimestamp() < time()) {
            throw new AutozNetworkSDKException('Inspection of access token metadata shows that the access token has expired.', 401);
        }
    }
